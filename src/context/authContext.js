...
export const AuthContextProvider = ({ children }) => {
  ...
  const initializeUserFromStorage = () => {
    try {
      ...
      if (storedUser) {
        const parsed = JSON.parse(storedUser);
        // Ensure the checks here match the structure you actually store
        // If parsed.user.id is missing, this will return null and force re-login
        if (parsed && parsed.token && parsed.user && parsed.user.id) {
          // This sessionStorage logic is for detecting a fresh browser start vs. a refresh.
          // It's not the cause of your current redirect loop if login just happened.
          if (!sessionStorage.getItem("activeSession")) {
            console.log(
              "AuthContext: No active session marker found in sessionStorage. Assuming fresh start, clearing localStorage for re-login."
            );
  ...
  const login = async (inputs) => {
    console.log("AuthContext - login function called with inputs:", inputs);
    try {
      // Make sure this path is correct relative to your baseURL in makeRequest
      // If makeRequest's baseURL is "http://34.219.141.197:8080/api/", then path is "auth/login"
      const res = await makeRequest.post("auth/login", inputs);
      const jwtToken = res.data; // Assuming backend sends just the JWT string directly
      console.log("AuthContext - API response received (raw JWT):", jwtToken);
      ...
      // --- CRITICAL MODIFICATION AREA ---
      // Adjust these lines based on the EXACT field names in your JWT payload from jwt.io
      const userToStore = {
        token: jwtToken,
        user: {
          id: decodedUser.userId || decodedUser.id, // Use actual claim name, e.g., 'userId' or 'id'
          username: decodedUser.username || decodedUser.sub, // Use actual claim name, e.g., 'username' or 'sub'
          name: decodedUser.name || decodedUser.username || decodedUser.sub, // Use actual claim name
          profilePic: decodedUser.profilePic || decodedUser.profilePicUrl || "", // Use actual claim name
        },
      };
      // --- END CRITICAL MODIFICATION AREA ---
